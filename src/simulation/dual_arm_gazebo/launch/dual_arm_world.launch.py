"""
Dual-arm simulation, complete.

Starts Gazebo with the world, bridges /clock, then hands over to
spawn_dual_arm.launch.py for the robot itself: robot_state_publisher, the Gazebo
...
Usage:
    ros2 launch dual_arm_gazebo dual_arm_world.launch.py
    ros2 launch dual_arm_gazebo dual_arm_world.launch.py gui:=false
    ros2 launch dual_arm_gazebo dual_arm_world.launch.py simulation_world:=/path/to.sdf
"""
...
def generate_launch_description():
    pkg_dual_arm_gazebo = FindPackageShare('dual_arm_gazebo')
    ...
    declared_arguments = [
        ...
        DeclareLaunchArgument(
            'z',
            default_value='0.855',
            # urdf_base sits at the waist with every link extending downward (arms to
            # z=-0.83). Spawning too low buries the collision bodies in the ground plane,
            # which stalls DART.
    ...
    gz_resource_path = SetEnvironmentVariable(
        ...
        value=os.pathsep.join([
            os.path.join(get_package_prefix('dual_arm_description'), 'share'),
    ...
    # /clock, so everything downstream can run on simulation time.
    clock_bridge = Node(
        package='ros_gz_bridge',
        executable='parameter_bridge',
        name='gz_ros_bridge',
        parameters=[{
            'config_file': PathJoinSubstitution([pkg_dual_arm_gazebo, 'config', 'bridge.yaml'])
        }],
        output='screen',
    )

    # robot_state_publisher + create + joint_state_broadcaster + the four controllers.
    spawn_dual_arm = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            PathJoinSubstitution([pkg_dual_arm_gazebo, 'launch', 'spawn_dual_arm.launch.py'])
        ),
        launch_arguments={
