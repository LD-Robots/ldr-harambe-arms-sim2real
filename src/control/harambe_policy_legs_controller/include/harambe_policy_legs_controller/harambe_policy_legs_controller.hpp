...
private:
  ...
  bool dry_run_{false};
  // fake_imu: feed the policy an IDEAL floating-base state — base_lin_vel=0,
  // gyro=0, projected_gravity=[0,0,-1] (perfectly upright, still, no noise/
  // latency). Diagnostic ONLY: isolates whether the thrashing is driven by the
  // IMU + velocity-estimator channels. The policy then gets NO tilt/velocity
  // feedback, so it CANNOT balance — run supported / in dry_run.
